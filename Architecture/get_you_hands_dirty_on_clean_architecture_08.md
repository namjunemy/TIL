...
우선, 일반적으로 많은 상용구 코드가 발생한다. 코드 양을 줄이기 위해 시중에 나와 있는 많은 매핑 프레임워크 중 하나를 사용하더라도 일반적으로 모델 간의 매핑을 구현하는 데 많은 시간이 걸린다. 이는 부분적으로 매핑 로직 디버깅이 고통스럽다는 사실 때문이다. 특히 일반 코드 및 리플렉션 레이어 뒤에 내부 작동을 숨3기는 매핑 프레임워크를 사용할 때 그렇다.
