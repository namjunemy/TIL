...
싱글 파일 컴포넌트 구조로 변경 된 후의 화면 구성입니다. 크롬의 Vue Devtools를 이용해서 Vue.js의 컴포넌트 구조를 확인할 수 있습니다. Root 컴포넌트 아래에 Vue가 렌더링을 시작하는 App 컴포넌트가 존재합니다. 그 아래에 Navbar와 Sidebar, Main 3개의 컴포넌트가 동일 레벨에서 렌더링 됩니다.

Main 컴포넌트에는 컴포넌트의 전환을 위해서 라우터가 등록되어 있고, 사용자의 선택에 따라서 해당 컴포넌트가 Main 컴포넌트 아래에 동적으로 그려집니다. Main 컴포넌트 아래에 다시 2개의 Child 컴포넌트....
